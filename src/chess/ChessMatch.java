...
public class ChessMatch {
    ...
    public ChessPiece performChessMove(ChessPosition  sourcePosition, ChessPosition targetPosition) {
        ...
        if (movedPiece instanceof Pawn) {
            if ((movedPiece.getColor() == Color.RED && target.getRow() == 0) || (movedPiece.getColor() == Color.YELLOW && target.getRow() == 7)) {
    ...
    private ChessPiece newPiece(String type, Color color) {
        ...
        if (type.equals("N")) return new Bishop(board, color);
        if (type.equals("Q")) return new Bishop(board, color);
    ...
    private Piece makeMove(Position source, Position target) {
        ...
        // #specialmove castling quennside move
        if (p instanceof King && target.getColumn() == source.getColumn() - 2) {
    ...
    private boolean testCheck(Color color) {
        ...
        List<Piece> oppenentPieces = piecesOnTheBoard.stream().filter(x -> ((ChessPiece)x).getColor() == opponent(color)).toList();
